...
export type Quad3 = Tuple<string, 3>;
export type Quad4 = Tuple<string, 4>;
export type Quads = (Quad3 | Quad4)[];
export type NQuadObj = { s: string; p: string; o: string; l?: string };
...
export interface GPath {
  ...
  all(): Promise<any[]>;
  /** Alias for intersect. */
  and(path: GPath): GPath;
  /** Alias for tag. */
  as(...tags: string[]): GPath;
...
the net result being a virtual predicate followed in the reverse direction. Starts at the end of the morphism and follows it backwards (with appropriate flipped directions) to the g.M() location. */
  ...
  followRecursive(path: GPath): GPath;
   ...
   */
  forEach(callback: (data: { id: string; [key: string]: any }) => void): void;
  forEach(
    limit: number,
    callback: (data: { id: string; [key: string]: any }) => void
  ): void;
  /** Alias for forEach. */
  map(callback: (data: { id: string; [key: string]: any }) => void): void;
  map(
    limit: number,
    callback: (data: { id: string; [key: string]: any }) => void
  ): void;
...
but do not follow the path, merely filter the possible paths. Usually useful for starting with all nodes, or limiting to a subset depending on some predicate/value pair.
  ...
  */
  has(predicate: string, object: string): GPath;
  /** The same as Has, but sets constraint in reverse direction. */
  hasR(predicate: string, object: string): GPath;
   ...
   */
  in: (
    predicatePath?: GPath | string | string[] | null | undefined,
    ...tags: (string[] | string)[]
  ) => GPath;
   ...
   */
  is(node: string, ...nodes: string[]): GPath;
...
function regex(expression: string, includeIRIs?: boolean): RegexFilter;
